...
internal sealed class BulkInsertUtilityTests
{
	...
	{
		Invoking(() => BulkInsertUtility.GetBulkInsertCommands("", SqlParamSource.Empty, [Sql.NamedParamsFromDto(new { foo = 1 })]).ToList())
	...
	{
		Invoking(() => BulkInsertUtility.GetBulkInsertCommands("VALUE (@foo)...", SqlParamSource.Empty, [Sql.NamedParamsFromDto(new { foo = 1 })]).ToList())
			.Should().Throw<ArgumentException>();
	}
	...
	{
		Invoking(() => BulkInsertUtility.GetBulkInsertCommands("1VALUES (@foo)...", SqlParamSource.Empty, [Sql.NamedParamsFromDto(new { foo = 1 })]).ToList())
			.Should().Throw<ArgumentException>();
	}
	...
	{
		Invoking(() => BulkInsertUtility.GetBulkInsertCommands("VALUES (@foo)... VALUES (@foo)...", SqlParamSource.Empty, [Sql.NamedParamsFromDto(new { foo = 1 })]).ToList())
			.Should().Throw<ArgumentException>();
	}
	...
	public void ZeroBatchSize_Throws()
	{
		Invoking(() => BulkInsertUtility.GetBulkInsertCommands("VALUES (@foo)...",
				SqlParamSource.Empty, [Sql.NamedParamsFromDto(new { foo = 1 })], new BulkInsertSettings { MaxRowsPerBatch = 0 }).ToList())
			.Should().Throw<ArgumentException>();
	}
	...
	{
		Invoking(() => BulkInsertUtility.GetBulkInsertCommands("VALUES (@foo)...",
				SqlParamSource.Empty, [Sql.NamedParamsFromDto(new { foo = 1 })], new BulkInsertSettings { MaxRowsPerBatch = -1 }).ToList())
			.Should().Throw<ArgumentException>();
	}
	...
	public void DifferentParameters()
	{
		var commands = BulkInsertUtility.GetBulkInsertCommands("VALUES (@a, @b)...",
			Sql.NamedParamsFromDto(new { a = 1, b = 2 }),
