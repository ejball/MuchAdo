...
internal sealed class SqlSyntaxTests
{
	...
	public void EmptySql()
	{
		...
		text.Should().Be("");
		EnumerateTuples(parameters).Should().Equal();
	...
	public void ParamSql()
	{
		...
		text.Should().Be("@ado1");
		EnumerateTuples(parameters).Should().Equal(("ado1", "xyzzy"));
	}
	...
	public void NamedParamSql()
	{
		...
		EnumerateTuples(parameters).Should().Equal(("abccb", "xyzzy"));
	}
	...
	public void ListSql()
	{
		...
		EnumerateTuples(parameters).Should().Equal(("ado1", "one"), ("ado2", "two"));
	}
	...
	public void ParamListSqlStrings()
	{
		...
		EnumerateTuples(parameters).Should().Equal(("ado1", "one"), ("ado2", "two"), ("ado3", "three"));
	}
	...
	public void ParamListSqlNumbers()
	{
		...
		EnumerateTuples(parameters).Should().Equal(("ado1", 1), ("ado2", 2));
	}
	...
	public void ParamListSqlMixedNumbers()
	{
		...
		text.Should().Be("@ado1, @ado2");
		EnumerateTuples(parameters).Should().Equal(("ado1", 1), ("ado2", 2L));
	}
	...
	public void ParamListSqlMixedObjects()
	{
		...
		text.Should().Be("@ado1, @ado2, @ado3");
		EnumerateTuples(parameters).Should().Equal(("ado1", "one"), ("ado2", 2), ("ado3", null));
	}
	...
	public void FormatImplicitParam()
	{
		...
		EnumerateTuples(parameters).Should().Equal(("ado1", 42), ("ado2", -42));
	...
	public void FormatSql(int? id)
	{
		...
		if (id is null)
		{
			...
			EnumerateTuples(parameters).Should().Equal(("ado1", limit));
		}
		else
		{
			text.Should().Be("select * from widgets where id = @ado1 limit @ado2");
			EnumerateTuples(parameters).Should().Equal(("ado1", id), ("ado2", limit));
		}
	}
	...
	public void FormatRawString()
	{
		...
		EnumerateTuples(parameters).Should().BeEmpty();
	}
	...
	public void SameParamTwice()
	{
		...
		EnumerateTuples(parameters).Should().Equal(("ado1", id), ("ado2", name), ("ado3", desc), ("ado4", name));
	}
	...
	public void LikeParamStartsWithSql()
	{
		...
		text.Should().Be("@ado1");
		EnumerateTuples(parameters).Should().Equal(("ado1", "xy\\_zy%"));
	}
	...
	public void ColumnNamesAndValuesSql()
	{
		...
		EnumerateTuples(parameters).Should().Equal(("ado1", item.Id), ("ado2", item.DisplayName), ("ado3", item.IsActive));
	}
	...
	public void ColumnNamesAndValuesWhereSql()
	{
		...
		text.Should().Be("""
			...
			values (@ado1);
			""");
		EnumerateTuples(parameters).Should().Equal(("ado1", item.DisplayName));
	}
