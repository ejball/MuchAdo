...
public abstract class Sql
{
	...
	{
		var commandBuilder = new SqlCommandBuilder(SqlSyntax.Ansi);
		Render(commandBuilder);
		return commandBuilder.Text;
	}

	internal abstract void Render(SqlCommandBuilder builder);
	...
	{
		internal override void Render(SqlCommandBuilder builder) => builder.AppendParameterValue(this, builder.Syntax.EscapeLikeFragment(prefix) + "%");
	}

	private sealed class NameSql(string identifier) : Sql
	{
		internal override void Render(SqlCommandBuilder builder) => builder.AppendText(builder.Syntax.QuoteName(identifier));
	}
	...
	{
		internal override void Render(SqlCommandBuilder builder) => builder.AppendParameterValue(this, value);
	}
	...
	{
		internal override void Render(SqlCommandBuilder builder) => builder.AppendText(text);
	}
}
